...
- Since, value of probability is between 0 and 1. Therefore, if the value of $$P(x)$$ is close to 1 then information is close to 0 and if value of $$P(x)$$ is close to 0 then information gain is very high. 

- Henceforth, an unlikely event has very high information and a likely event has very low information. Consider following examples as : 
	> We have our tutorial deadline some or the other day. --> This has zero information beacuse it's an evident event.
	>
	> We got our deadline extended. --> This has high information because it is very unlikely event :) 

- The average over the information is known as entropy. Entropy can be calculated as $$H = -\sum P(x)*log(P(x))$$, which is just the expectation of information $$I$$. 

- Now to carry out our discussion further, we introduce a metric Kullback–Leibler divergence (aka KL divergence) which helps us two measure how dissimilar are two given distributions.

- KL Divergence 
	> Assuming there are two different distributions $$P$$ and $$Q$$. Then KL Divergence is the measure of dissimilarity between $$P$$ and $$Q$$. 
	> 
	> KL-Divergence is a concept which is closely related to Information and Entropy. KL-Divergence between $$P$$ and $$Q$$ is almost equal to difference in entropy of $$P$$ and $$Q$$. 
	> 
	> KL divergence is defined as difference of cross entropy and entropy. It is always evaluated with respect to one of the two distributions. Henceforth its an assymetric measure (so $$KL(P\|Q) \neq KL(Q\|P)$$).
	>
	<!--Therefore, $$KL(P\|Q) \sim -\sum Q(x)*log(Q(x)) + \sum P(x)*log(P(x))$$ i.e. Difference in information obtained from distribution P and Q. Therefore, if both distributions are similar then entropy is minimum. However, KL-Divergence is calculated with respect to one distribution. -->
	> KL-Divergence of Q with respect to P can be forumated as $$KL(P\|Q) = -\sum P(x)*log(Q(x)) + \sum P(x)*log(P(x))$$.
	>
	> One can interpret KL divergence as <!--here average information of Q is calculated with repsect to P.--> computing the difference in average bits required to encode samples of **$$P(X)$$ using $$Q(X)$$ i.e $$-P(x)*log(Q(x)) $$** and average bits required to encode samples of **$$P(X)$$ using $$P(X)$$ i.e $$-P(x)*log(P(x))$$**.Assume here X used is some random variable. 

- The two important properties of KL-Divergence are as follows 
	> KL-Divergence is always greater than equal to 0 (because cross entropy $$\geq$$ entropy)
...
- Since, Variational technique is a technique in inference in graphical model so, let us start with graphical models. Suppose, we have $$z$$ which is a hidden variable and $$x$$ is an observation. We would like to compute posterior $$P(z \vert x)$$ which can be written as follows : 

$$P(z \vert x) = \frac{P(x \vert z) * P(z)}{P(x)} = \frac{P(x,z)}{P(x)} $$

- Computing the marginal distribution $$P(x)$$ is quite complicated. As $$P(x) = \int P(x \vert z)*p(z) dz$$ and this integral is intractable especially in high dimensional space(In [problem setting](#problem-setting) section we have already shown this). 

- Therefore computing the marginal is one of obstacle in graphical models. Roughly there are two main approaches for calculating this integral. 
	> 1. Monte Carlo approach (compute the integral by monte carlo integral using sampling).
	>
	> 2. Variational Inference. We will use variational Inference.

- As we know computing $$P(z \vert x)$$ is not tractable therefore we try to approximate $$P(z \vert x)$$ with another distribution $$Q(z)$$. If $$Q$$ is choosen to be a tractable distribution and if this tractable distribution is made close to the distribution of $$P(z \vert x)$$ then the problem is solved.
...
- Let us assume $$Q(z \vert x)$$ is a function is obtained from a neural network which takes input $$x$$ and maps it to $$z$$.Further assume that $$P(x \vert z)$$ is another neural network which takes this $$z$$ and maps it to $$x$$. 

- We also assume that $$z$$ follows a univariate gaussian distribution.
	> Some papers calls assuming such prior as way of regularization in VAE. But here we argue that this is not a regularization rather its misused here.
	>
	> Regularization by definition means a way of introducing our intuition or belief on **parameters** of model where as if we revisit our assumption i.e "Assume that $$z$$ follows a univariate gaussian distribution". So here we don't put belief on model parameters rather we model our belief on the data distribution followed in latent space. Therefore its not correct to say that KL divergence regularizes the model.

- Coming back to our loss objective, 
...
Variational autoencoders imposes uncertainty in the hidden layer via reparameterization trick whereas denoising autoencoder learns regularized autoencoders by adding some noise in the input. Denoising variational autoencoder is the blend of the above two. The approach followed in the denoising autoencoder is to add noise to the input and then send the input to the vanilla VAE network. This input is then further reconstructed to eliminate the noise from the input. The corruption distribution in Denoising VAE is $$p(\hat{x} \vert x)$$. The original posterior distribution (Encoder Distribution) is $$q_\phi (z \vert \hat{x})$$ and the posterior distribution for denoising is $$\hat{q_\phi}(z \vert x) = \int q_\phi (z \vert \hat{x})p(\hat{x}\vert x)\hat{dx}$$. As the model is trained to reconstruct the clean nput from a noisy input. therefore we expect our DVAE to be more robust to outliers. In contrast to vanilla VAE/GAN and conditinal VAE/GAN, the DVAE may have better representation learning since it generates a clean output from a noisy input. Stable training when compared to GAN variants like C-GAN.
