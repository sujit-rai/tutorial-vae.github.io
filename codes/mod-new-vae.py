...
#To do
#1) visualize conv nd deconv
#2) trainable parameters add to conv layers
...
def decoder(Z,isTrainable=True,reuse=False,name='decoder'):
    with tf.variable_scope(name) as scope:
        ...
        #Instead of using resize_images followed by conv layer we can use fractionally strided convolutional also 
...
def generateSample():
    with tf.Session() as sess:
        ...
        string = os.path.dirname(os.path.abspath(__file__))+'/'+model_directory+'/model_'+str(n_epochs-2); 
        ...
        print ("Model loaded from ",string);
        saver = tf.train.Saver();
        ...
        for i in range(n):
            #seed_vale += 1;
            rand1 = tf.random_normal([1,z_dim]).eval();
            #seed_vale += 1;
            rand2 = tf.random_normal([1,z_dim]).eval();
            values_ = tf.lin_space(0.0,1.0,n).eval();
            sample = tf.random_normal([n,z_dim]).eval();

            for k in range(n):
                sample[k,:] = (rand1 * values_[k] + rand2 * (1-values_[k]));
        ...
        plt.figure(figsize=(n, n));
        plt.imshow(reconstructed, origin="upper", cmap="gray");
        plt.savefig(os.path.dirname(os.path.abspath(__file__))+'/interpolation-img.png');
        plt.close();
        ...
        # plt.figure(figsize=(n, n));
        # plt.imshow(reconstructed, origin="upper", cmap="gray");
        # plt.savefig(os.path.dirname(os.path.abspath(__file__))+'/gen-img.png');
        # plt.close();
